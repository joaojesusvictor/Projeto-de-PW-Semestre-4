...
cs('.cart--finalizar').forEach((item)=>{
    item.addEventListener('click', closeModal);
});

c('.modelsInfo--qtmenos, .cart--finalizar').addEventListener('click', ()=>{
    if(modalQt > 1) {
        modalQt--;
        c('.modelsInfo--qt').innerHTML = modalQt;
    }
...
function updateCart() {
    ...
    if(cart.length > 0) {
        ...
        cart.map((itemCart, index)=>{
            ...
            cartItem.querySelector('.cart--item-qtmenos, .cart--finalizar').addEventListener('click',()=>{
                if(itemCart.qt > 1) {
                    itemCart.qt--
                } else {
                    cart.splice(index, 1);
